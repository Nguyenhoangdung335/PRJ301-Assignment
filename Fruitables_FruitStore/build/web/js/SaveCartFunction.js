...
saveCartBt.addEventListener('click', function(event) {
    ...
    if (user === null) {
        messageAppear('Please log in to save shopping cart');
    } else if (IsEmpty()) {
        messageAppear('There is no item to save, please try again with item in cart');
    } else {
        ...
        for (let input of isSentInput) {
            ...
            if (name === 'txtQuantity' && parseFloat(value) <= 0) {
                messageAppear('Quantity must not be zero when saving cart, please try again!');
                return;
            }            
