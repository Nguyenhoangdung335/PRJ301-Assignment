...
makeOrderBt.addEventListener('click', function(event) {
    ...
    if (user === null) {
        messageAppear('Please log in to make order');
    } else if (IsEmpty()) {
        messageAppear('There is no item to make order, please try again with item in cart');
    } else {
        ...
        for (let input of isSentInput) {
            ...
            if (name === 'txtQuantity' && parseFloat(value) <= 0) {
                messageAppear('Quantity must not be zero when making order, please try again!');
                return;
            }
            formData += name + '=' + value + '&';
        }
